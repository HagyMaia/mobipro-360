...
export class DriverService {
    ...
    public static async registerDriver(
        ...
        let driverPayload: Record<string, any> = {
            ...
            cep: data.zipCode?.trim() || '',
            rua: data.street?.trim() || '',
            numero: data.number?.trim() || '',
            complemento: data.complement?.trim() || '',
            bairro: data.neighborhood?.trim() || '',
            cidade: data.city?.trim() || 'Manaus',
            estado: data.state?.trim() || 'AM',
            tipo_motorista: dType,
            driver_type: dType,
            perfil_motorista: dType,
            categoria_motorista: dType,
            categoria_tipo: dType === 'EMPRESA' ? 'empresa' : 'particular',
            recebe_voucher: true,
            recebe_particular: dType !== 'EMPRESA',
            ...
            dados_pessoais_status: 'Pendente',
            personal_data_status: 'Pendente',
            dados_empresa_status: 'Pendente',
            company_data_status: 'Pendente',
            work_status: 'OFFLINE',
